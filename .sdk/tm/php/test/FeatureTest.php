...
class FtCtrl extends HtmlCreatorControl
{
...
class FtClient
{
    ...
    private HtmlCreatorUtility $utility;

    public function __construct(HtmlCreatorUtility $utility, array $options)
    {
    ...
    public function get_utility(): HtmlCreatorUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (HtmlCreatorContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public HtmlCreatorContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (HtmlCreatorContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = HtmlCreatorConfig::make_feature($name);
    ...
    private static function buildUrl(HtmlCreatorSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof HtmlCreatorError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new HtmlCreatorResponse($fetched);
        }
    ...
    private function fail(HtmlCreatorContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (HtmlCreatorContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
