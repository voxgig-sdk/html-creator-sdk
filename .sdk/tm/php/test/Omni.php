...
//    only - a typed HtmlCreatorContext there would make every ctx
...
final class HtmlCreatorOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new HtmlCreatorContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new HtmlCreatorSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new HtmlCreatorResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new HtmlCreatorError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new HtmlCreatorResponse($clean['response']);
    ...
    public static function err_from_map($m): ?HtmlCreatorError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new HtmlCreatorError(is_string($code) ? $code : '', $msg);
    }
