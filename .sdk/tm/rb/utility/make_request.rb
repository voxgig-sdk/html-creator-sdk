...
module HtmlCreatorUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = HtmlCreatorResponse.new({})
    ...
    if fetch_err
      ...
      response = HtmlCreatorResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = HtmlCreatorResponse.new(fetched)
    else
