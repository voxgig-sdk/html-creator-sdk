...
func (sdk *HtmlCreatorSDK) OptionsMap() map[string]any {
...
func (sdk *HtmlCreatorSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *HtmlCreatorSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *HtmlCreatorSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *HtmlCreatorSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *HtmlCreatorSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *HtmlCreatorSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("HtmlCreatorSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *HtmlCreatorSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *HtmlCreatorSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("HtmlCreatorSDK: graphql: %s", msg)
		res["graphql"] = errors
	}
