...
import (
	...
	omni "github.com/voxgig-sdk/html-creator-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.HtmlCreatorError); is {
				out := map[string]any{
					"name":    "HtmlCreatorError",
...
func omniCtx(arg any, client *sdk.HtmlCreatorSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.HtmlCreatorSDK); is {
			client = live
