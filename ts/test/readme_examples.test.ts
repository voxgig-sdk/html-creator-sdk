...
//     (\`new HtmlCreatorSDK(...)\` / \`HtmlCreatorSDK.test(...)\` become
//     \`HtmlCreatorSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"html_document":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new HtmlCreatorSDK(...)\` and any
// \`HtmlCreatorSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+HtmlCreatorSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/HtmlCreatorSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
