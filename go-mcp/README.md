...
[MCP](https://modelcontextprotocol.io) server exposing the HtmlCreator SDK as
two agent tools — `html-creator_list` and `html-creator_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/html-creator-mcp)
make build

# 2. Provide credentials via the environment
export HTML_CREATOR_APIKEY=sk_live_xxx
...
```jsonc
// html-creator_list: first page of records
{ "entity": "html_document" }
{ "entity": "html_document", "query": { } }

// html-creator_load: one record by id
{ "entity": "html_document", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/html-creator-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user html-creator \
     -- "$PWD"/dist/*/html-creator-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `html-creator_list` and `html-creator_load` tools now appear
   in new sessions. Ask the agent to *"list html_document using html-creator"*
   and it calls `html-creator_list` with `{"entity":"html_document"}`.
...
```sh
export HTML_CREATOR_APIKEY=sk_live_xxx            # API key
export HTML_CREATOR_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `html-creator_list` tool
...
### Call the `html-creator_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `html-creator_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `html-creator_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `HTML_CREATOR_APIKEY` | API key sent with every request. |
| `HTML_CREATOR_BASE` | Optional override of the API base URL. |
