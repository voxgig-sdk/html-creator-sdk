...
The HTML Creator API allows users to create and edit HTML code with an intuitive interface, supporting advanced JavaScript functionality.

Learn more about HTML Creator API at [html-creator.com](https://html-creator.com).

This is an unofficial SDK for the HTML Creator public API, generated by Voxgig with [`@voxgig/sdkgen`](https://github.com/voxgig/sdkgen). It is not affiliated with, endorsed by, or sponsored by the upstream API provider.
...
This SDK exposes the API as a small set of **semantic entities** — HtmlDocument — that you
...
const htmldocument = await client.HtmlDocument().create({
  content: {},
})
```
...
const client = HtmlCreatorSDK.test({
  entity: {
    html_document: {
      test01: { id: 'test01', content: {} },
    },
  },
})
const htmldocument = await client.HtmlDocument().create({ content: {} })
// htmldocument is the HtmlDocument entity, populated with mock data
...
```go
client := sdk.Test()
result, err := client.HtmlDocument(nil).Create(
...
| --- | --- | --- |
| **HtmlDocument** | The HtmlDocument entity (create). | `/html/create` |
...
- Upstream API: [https://html-creator.en.uptodown.com/android](https://html-creator.en.uptodown.com/android)
