...
boru-driven command-line client **and** interactive REPL for the HtmlCreator
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/html-creator-cli)
make build

# 2. See usage (words, entities, env vars)
./html-creator-cli --help
...
HTML_CREATOR_BASE=https://api.example.com ./html-creator-cli --help
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/html-creator-cli
   ```
...
Run with no arguments to open a REPL (prompt `html-creator>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `html_document`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/html-creator-cli`. |
...
(`./html-creator-cli <expr>`) and interactively (the REPL), and expressions compose the
