// Command HtmlCreator-cli is a CLI + REPL for the HtmlCreator SDK,
...
const prompt = "html-creator"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "html_document"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      HTML_CREATOR_APIKEY (api key), HTML_CREATOR_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: HTML_CREATOR_APIKEY carries the API key and
	// HTML_CREATOR_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("HTML_CREATOR_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("HTML_CREATOR_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewHtmlCreatorSDK(opts)
