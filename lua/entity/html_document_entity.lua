...
local HtmlDocumentEntity = {}
HtmlDocumentEntity.__index = HtmlDocumentEntity


function HtmlDocumentEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, HtmlDocumentEntity)
  self._name = "html_document"
  self._client = client
...
function HtmlDocumentEntity:get_name()
  return self._name
end


function HtmlDocumentEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return HtmlDocumentEntity.new(self._client, opts)
end
...
function HtmlDocumentEntity:mark_deleted()
  self._deleted = true
end


function HtmlDocumentEntity:deleted()
  return true == self._deleted
end


function HtmlDocumentEntity:data_set(args)
  if args ~= nil then
...
function HtmlDocumentEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function HtmlDocumentEntity:match_set(args)
  if args ~= nil then
...
function HtmlDocumentEntity:match_get()
...
function HtmlDocumentEntity:stream(action, args, callopts)
...
function HtmlDocumentEntity:_run_op(ctx, post_done)
  local utility = self._utility
