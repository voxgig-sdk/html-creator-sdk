...
class HtmlDocumentEntity
  ...
  def make
    opts = @_entopts.dup
    HtmlDocumentEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = HtmlCreatorHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = HtmlCreatorHelpers.to_map(VoxgigStruct.clone(args)) || {}
    ...
    _run_op(ctx) do
      if ctx.result
        if ctx.result.resdata
          @_data = HtmlCreatorHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
