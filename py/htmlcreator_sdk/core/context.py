...
from htmlcreator_sdk.core.control import HtmlCreatorControl
...
from htmlcreator_sdk.core.helpers import get_ctx_prop, to_map
...
class HtmlCreatorContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        if isinstance(sp, HtmlCreatorSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, HtmlCreatorResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, HtmlCreatorResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def make_error(self, code, msg):
        return HtmlCreatorError(code, msg, self)
