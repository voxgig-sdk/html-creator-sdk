...
class HtmlCreatorTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            HtmlCreatorTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = HtmlCreatorTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = HtmlCreatorTestRunner.getenv("HTML_CREATOR_TEST_LIVE")
        override = HtmlCreatorTestRunner.getenv("HTML_CREATOR_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = HtmlCreatorTestRunner.getenv(key)
        ...
        explain = HtmlCreatorTestRunner.getenv("HTML_CREATOR_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if HtmlCreatorTestRunner._test_control is not None:
            return HtmlCreatorTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                HtmlCreatorTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            HtmlCreatorTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = HtmlCreatorTestRunner.load_test_control()
...
def load_env_local():
    HtmlCreatorTestRunner.load_env_local()


def env_override(m):
    return HtmlCreatorTestRunner.env_override(m)


def entity_data(v):
    return HtmlCreatorTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return HtmlCreatorTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return HtmlCreatorTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return HtmlCreatorTestRunner.load_test_control()


def live_client_options():
    return HtmlCreatorTestRunner.live_client_options()


def live_delay_ms():
    return HtmlCreatorTestRunner.live_delay_ms()
