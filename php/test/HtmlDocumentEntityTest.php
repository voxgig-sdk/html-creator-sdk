...
class HtmlDocumentEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set HTML_CREATOR_TEST_HTML_DOCUMENT_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $html_document_ref01_data = Helpers::to_map(is_object($html_document_ref01_data_result) && method_exists($html_document_ref01_data_result, 'data_get') ? $html_document_ref01_data_result->data_get() : $html_document_ref01_data_result);
...
function html_document_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("HTML_CREATOR_TEST_HTML_DOCUMENT_ENTID");
